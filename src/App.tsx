...
import { Oscilloscope, PianoKeyboard, WaveformDisplay } from "./components";
import { useAudioContext, useKeyboardInput } from "./hooks";
import { AudioRecorder, GranularSynth, MIDIHandler } from "./lib/audio";
...
const OpenCollidoscopeApp: React.FC = () => {
  ...
  const [granularSynth, setGranularSynth] = useState<GranularSynth | null>(
    null,
  );
  const [audioRecorder, setAudioRecorder] = useState<AudioRecorder | null>(
    null,
  );
  const [_midiHandler, setMidiHandler] = useState<MIDIHandler | null>(null);
  ...
  const calculateMidiNoteRatio = useCallback((midiNote: number): number => {
    ...
    if (distanceFromCenter < 0) {
      ...
      return Math.pow(0.5, octaves) / (chromaticRatios[intervals] || 1);
    } else {
      const octaves = Math.floor(distanceFromCenter / 12);
      const intervals = distanceFromCenter % 12;
      return Math.pow(2, octaves) * (chromaticRatios[intervals] || 1);
    }
  }, []);

  // Audio initialization
  const initializeAudioComponents = useCallback(() => {
    ...
    try {
      ...
      midi.onControlChange = (controller, value) =>
  ...
  const handleNoteOn = useCallback(
    (midiNote: number, velocity: number = 0.8) => {
    ...
    (controller: number, value: number) => {
      switch (controller) {
        case MIDI_CC.SELECTION_SIZE: {
          const selectionSize =
            Math.floor((value / 127) * AUDIO_CONSTANTS.MAX_SELECTION_CHUNKS) +
            1;
          setAppState((prev) => ({
            ...prev,
        ...
        case MIDI_CC.LOOP_TOGGLE: {
          ...
          if (granularSynth) {
            if (isLooping) {
              granularSynth.loopOn();
            } else {
              granularSynth.loopOff();
            }
          }
          break;
        }
  ...
  const handlePitchBend = useCallback(
    (value: number) => {
      ...
      const maxPosition =
        AUDIO_CONSTANTS.DEFAULT_CHUNKS - appState.granularParams.selectionSize;
  ...
  // Wrapper for async toggleRecording function
        ...
        <main className="app-content">
          ...
          <section className="granular-controls">
            <div className="control-group">
              <label htmlFor="selection-size">
                Selection Size: {appState.granularParams.selectionSize}
              </label>
              <input
                id="selection-size"
                type="range"
            ...
            <div className="control-group">
              <label htmlFor="grain-duration">
                Grain Duration:{" "}
                {appState.granularParams.grainDurationCoeff.toFixed(1)}
              </label>
              <input
                id="grain-duration"
                type="range"
            ...
            <div className="control-group">
              <label htmlFor="filter-cutoff">
                Filter Cutoff: {appState.granularParams.filterCutoff}
              </label>
              <input
                id="filter-cutoff"
                type="range"
            ...
            <div className="control-group">
              <label htmlFor="master-volume">
                Master Volume: {appState.granularParams.masterVolume.toFixed(1)}
              </label>
              <input
                id="master-volume"
                type="range"
          ...
          <section className="waveform-section">
            <WaveformDisplay
              audioBuffer={appState.recordedBuffer || undefined}
              selectionStart={appState.granularParams.selectionStart}
              selectionSize={appState.granularParams.selectionSize}
          ...
          <section className="oscilloscope-section">
            <Oscilloscope
              ...
              isActive={appState.isAudioStarted}
            />
          </section>
          ...
          <section className="keyboard-section">
            <PianoKeyboard
              ...
              activeNotes={appState.pressedKeys}
            />
          </section>
          ...
          <section className="status-section">
            <div className="status-info">
              <span>
                Audio: {appState.isAudioStarted ? "Started" : "Stopped"}
              </span>
              <span>
                Recording: {appState.isRecording ? "Active" : "Inactive"}
              </span>
              <span>Loop: {appState.isLooping ? "On" : "Off"}</span>
              {appState.recordedBuffer && (
                <span>
                  Buffer: {appState.recordedBuffer.duration.toFixed(2)}s
                </span>
              )}
              {granularSynth && (
                <span>
                  Active Grains: {granularSynth.getActiveGrainCount()}
                </span>
              )}
            </div>
          </section>
