...
| 機能 | 説明 | Phase |
| --- | --- | --- |
| マイク入力 | `getUserMedia` でマイクから音声取得 | 1 |
| 録音 | 指定時間の録音、フェードイン/アウト、チャンク分割 | 1 |
| 波形表示 | チャンク min/max バー、選択範囲ハイライト、アニメーション | 1 |
| 選択操作 | 開始位置（ドラッグ）、サイズ（ホイール） | 1 |
| グラニュラーシンセ | グレイン生成、Hann 窓、ピッチ、ループ | 1 |
| フィルター | ローパス、指数的カットオフ制御 | 1 |
| ピアノ鍵盤 | 画面上の鍵盤 + PC キーボード | 1 |
| オシロスコープ | 再生中の出力波形をリアルタイム表示 | 1 |
| パーティクル | duration coeff に連動した視覚フィードバック | 1 |
| MIDI 入力 | Web MIDI API による外部コントローラー対応 | 1 |
| 設定 UI | 全パラメータの変更、プリセット、JSON 入出力 | 1 |
...
| スライダー | Selection Start, Selection Size, Duration, Filter |
| ボタン | Record, Loop ON/OFF |
| 設定パネル | 全パラメータ、プリセット、JSON 入出力 |
