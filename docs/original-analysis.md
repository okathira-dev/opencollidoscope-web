...
俯瞰図・スロット ID・Web 版投影は [hardware-layout.md](hardware-layout.md) を参照。要約:

| スロット ID | 部品 | MIDI | 機能 |
| --- | --- | --- | --- |
| `SLOT_FADER_FILTER` / `SLOT_FADER_DURATION` | 縦フェーダー ×2（オリジナル）/ 縦ノブ（新版） | CC7 / CC2 | ローパスカットオフ / グレイン持続係数 |
| `SLOT_WAVEJET` | アルミノブ + エンコーダー + SoftPot | CC1 + Pitch Bend | 選択サイズ / 選択開始 |
| `SLOT_RECORD` | 16mm 赤プッシュ（LED リング） | CC5 | 2 秒録音（`player_end`・マイク付近） |
| `SLOT_KEYBOARD` | USB MIDI キーボード | Note On/Off | ピッチ付き演奏（最大 6 ボイス） |
| `SLOT_LOOP_TOGGLE` / `SLOT_LOOP_PUSH` | トグル（オリジナル）/ プッシュ（新版） | CC4 | ループ ON/OFF |
